...
module viscoelastic_class
   ...
   type, extends(multiscalar) :: viscoelastic
      ...
      real(WP), dimension(:,:,:,:),   allocatable :: tsrC

      character(len=str_medium), dimension(:), allocatable :: tsrCname  !< Names of Tensor components
         ...
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  ...
                  coeff=(this%Lmax**2-3.00_WP)/(this%Lmax**2-(this%tsrC(i,j,k,1)+this%tsrC(i,j,k,4)+this%tsrC(i,j,k,6)))
                  if (present(trelaxfield)) then
                     trelax=trelaxfield(i,j,k)
                  else
                     trelax=this%trelax
                  end if
                  resSC(i,j,k,1)=-(coeff*this%tsrC(i,j,k,1)-1.0_WP)/trelax !< xx tensor component
                  resSC(i,j,k,2)=-(coeff*this%tsrC(i,j,k,2)-0.0_WP)/trelax !< xy tensor component
                  resSC(i,j,k,3)=-(coeff*this%tsrC(i,j,k,3)-0.0_WP)/trelax !< xz tensor component
                  resSC(i,j,k,4)=-(coeff*this%tsrC(i,j,k,4)-1.0_WP)/trelax !< yy tensor component
                  resSC(i,j,k,5)=-(coeff*this%tsrC(i,j,k,5)-0.0_WP)/trelax !< yz tensor component
                  resSC(i,j,k,6)=-(coeff*this%tsrC(i,j,k,6)-1.0_WP)/trelax !< zz tensor component
               end do
            end do
         end do
         ...
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  ...
                  coeff=1.0_WP-(this%tsrC(i,j,k,1)+this%tsrC(i,j,k,4)+this%tsrC(i,j,k,6))/this%Lmax**2
                  ...
                  resSC(i,j,k,1)=-coeff*(this%tsrC(i,j,k,1)-1.0_WP) !> xx tensor component
                  resSC(i,j,k,2)=-coeff*(this%tsrC(i,j,k,2)-0.0_WP) !> xy tensor component
                  resSC(i,j,k,3)=-coeff*(this%tsrC(i,j,k,3)-0.0_WP) !> xz tensor component
                  resSC(i,j,k,4)=-coeff*(this%tsrC(i,j,k,4)-1.0_WP) !> yy tensor component
                  resSC(i,j,k,5)=-coeff*(this%tsrC(i,j,k,5)-0.0_WP) !> yz tensor component
                  resSC(i,j,k,6)=-coeff*(this%tsrC(i,j,k,6)-1.0_WP) !> zz tensor component
               end do
            end do
         end do
         ...
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  ...
                  this%tsrC(i,j,k,1)=min(max(0.0_WP,this%tsrC(i,j,k,1)),this%Lmax**2)
                  this%tsrC(i,j,k,4)=min(max(0.0_WP,this%tsrC(i,j,k,4)),this%Lmax**2)
                  this%tsrC(i,j,k,6)=min(max(0.0_WP,this%tsrC(i,j,k,6)),this%Lmax**2)
                  this%tsrC(i,j,k,2)=min(max(-this%Lmax**2,this%tsrC(i,j,k,2)),this%Lmax**2)
                  this%tsrC(i,j,k,3)=min(max(-this%Lmax**2,this%tsrC(i,j,k,3)),this%Lmax**2)
                  this%tsrC(i,j,k,5)=min(max(-this%Lmax**2,this%tsrC(i,j,k,5)),this%Lmax**2)
                  ...
                  coeff=max(coeff,safety_margin*dt)                             !< Further clip based on current time step size for stability
                  coeff=1.0_WP/coeff                                            !< Inverse coeff
                  resSC(i,j,k,1)=resSC(i,j,k,1)-coeff*(this%tsrC(i,j,k,1)-1.0_WP) !> xx tensor component
                  resSC(i,j,k,2)=resSC(i,j,k,2)-coeff* this%tsrC(i,j,k,2)         !> xy tensor component
                  resSC(i,j,k,3)=resSC(i,j,k,3)-coeff* this%tsrC(i,j,k,3)         !> xz tensor component
                  resSC(i,j,k,4)=resSC(i,j,k,4)-coeff*(this%tsrC(i,j,k,4)-1.0_WP) !> yy tensor component
                  resSC(i,j,k,5)=resSC(i,j,k,5)-coeff* this%tsrC(i,j,k,5)         !> yz tensor component
                  resSC(i,j,k,6)=resSC(i,j,k,6)-coeff*(this%tsrC(i,j,k,6)-1.0_WP) !> zz tensor component
               end do
            end do
         end do
       ...
       case (lptt) ! Add relaxation source term for lPTT model
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  if (this%mask(i,j,k).ne.0) cycle                              !< Skip non-solved cells
                  coeff=1.00_WP+(this%elongvisc/(1.0_WP-this%affinecoeff))*((this%tsrC(i,j,k,1)+this%tsrC(i,j,k,4)+this%tsrC(i,j,k,6))-3.0_WP)
                  if (present(trelaxfield)) then
                     trelax=trelaxfield(i,j,k)
                  else
                     trelax=this%trelax
       ...
       case (eptt) ! Add relaxation source term for ePTT model
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  if (this%mask(i,j,k).ne.0) cycle                              !< Skip non-solved cells
                  coeff=exp(this%elongvisc/(1.0_WP-this%affinecoeff)*((this%tsrC(i,j,k,1)+this%tsrC(i,j,k,4)+this%tsrC(i,j,k,6))-3.0_WP))
                  if (present(trelaxfield)) then
                     trelax=trelaxfield(i,j,k)
                  else
                     trelax=this%trelax
