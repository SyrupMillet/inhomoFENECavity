...
module viscoelastic_class
   ...
   type, extends(multiscalar) :: viscoelastic
      ...
      real(WP), dimension(:,:,:,:),   allocatable :: SCrec2
   ...
   contains
      ...
      procedure :: get_relax_analytical2                    !< Calculate relaxation term based on a semi-analytical integration
...
contains
   ...
   subroutine get_CgradU_log(this,gradu,resSC,phi)
      implicit none
      class(viscoelastic), intent(inout) :: this
      real(WP), dimension(1:,1:,this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in)    :: gradU
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:,1:),    intent(inout) :: resSC
      ...
      do k=this%cfg%kmino_,this%cfg%kmaxo_
         do j=this%cfg%jmino_,this%cfg%jmaxo_
            do i=this%cfg%imino_,this%cfg%imaxo_
               ! Skip non-solved cells
               if (this%mask(i,j,k).ne.0) cycle
               if (phi(i,j,k) .lt. threshold) cycle
         ...
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  ...
                  coeff=(this%Lmax**2-3.00_WP)/(this%Lmax**2-(this%SCrec2(i,j,k,1)+this%SCrec2(i,j,k,4)+this%SCrec2(i,j,k,6)))
                  if (present(trelaxfield)) then
                     trelax=trelaxfield(i,j,k)
                  else
                     trelax=this%trelax
                  end if
                  resSC(i,j,k,1)=-(coeff*this%SCrec2(i,j,k,1)-1.0_WP)/trelax !< xx tensor component
                  resSC(i,j,k,2)=-(coeff*this%SCrec2(i,j,k,2)-0.0_WP)/trelax !< xy tensor component
                  resSC(i,j,k,3)=-(coeff*this%SCrec2(i,j,k,3)-0.0_WP)/trelax !< xz tensor component
                  resSC(i,j,k,4)=-(coeff*this%SCrec2(i,j,k,4)-1.0_WP)/trelax !< yy tensor component
                  resSC(i,j,k,5)=-(coeff*this%SCrec2(i,j,k,5)-0.0_WP)/trelax !< yz tensor component
                  resSC(i,j,k,6)=-(coeff*this%SCrec2(i,j,k,6)-1.0_WP)/trelax !< zz tensor component
               end do
            end do
         end do
   ...
   !> Assumes scalar being transported is ln(C)
   subroutine get_eigensystem(this,phi)
   ...
   subroutine get_eigensystem_SCrec(this,phi)
      use mathtools, only: eigensolve3
      implicit none
      class(viscoelastic), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in) :: phi
   ...
   !> Assume SC is log(phiC), SCrec is phiC
   !> Get C from SCrec
   subroutine getCFtensor(this, phi)
      implicit none
      class(viscoelastic), intent(inout) :: this
         ...
         do j=this%cfg%jmino_,this%cfg%jmaxo_
            do i=this%cfg%imino_,this%cfg%imaxo_
               ...
               if (phi1 .lt. threshold) then
                  this%SCrec2(i,j,k,:) = identity
               else
                  this%SCrec2(i,j,k,:) = this%SCrec(i,j,k,:)/phi1
               end if
            end do
         end do
      end do
   ...
   !> Add viscoelastic relaxation source based on semi analtical integration
   subroutine get_relax_analytical2(this,dt,trelaxfield, phi)
      use messager, only: die
      implicit none
      class(viscoelastic), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in), optional :: trelaxfield
      ...
      real(WP), intent(in) :: dt
      integer :: i,j,k
      real(WP) :: f,coeff,trelax, phi1

      ! dD/dt = - (1/relax) * [ f(c)*D - phi * I ]

      select case (this%model)
       case (oldroydb)
         ! Add relaxation source for Oldroyd-B (1/t_relax)(C-I)
         ! f(C) = 1
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  ...
                  coeff=-1.0_WP*(dt/trelax)
                  phi1=phi(i,j,k)

                  this%SCrec(i,j,k,1)=this%SCrec(i,j,k,1)*exp(coeff) + (1.00_WP-exp(coeff))*1.0_WP*phi1 !< xx tensor component
                  this%SCrec(i,j,k,2)=this%SCrec(i,j,k,2)*exp(coeff) + (1.00_WP-exp(coeff))*0.0_WP*phi1 !< xy tensor component
                  this%SCrec(i,j,k,3)=this%SCrec(i,j,k,3)*exp(coeff) + (1.00_WP-exp(coeff))*0.0_WP*phi1 !< xz tensor component
                  this%SCrec(i,j,k,4)=this%SCrec(i,j,k,4)*exp(coeff) + (1.00_WP-exp(coeff))*1.0_WP*phi1 !< yy tensor component
                  this%SCrec(i,j,k,5)=this%SCrec(i,j,k,5)*exp(coeff) + (1.00_WP-exp(coeff))*0.0_WP*phi1 !< yz tensor component
                  this%SCrec(i,j,k,6)=this%SCrec(i,j,k,6)*exp(coeff) + (1.00_WP-exp(coeff))*1.0_WP*phi1 !< zz tensor component
               end do
            end do
         end do
       case (fenep)
         ! FENE-P: relaxation (fC-I) with integration yielding D(t+dt)=D(t)*exp(-f dt/trelax)+(phi*I/f)*(1-exp(-f dt/trelax))
         ! f = (Lmax^2-3)/(Lmax^2 - tr(C))
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  ...
                  if (phi(i,j,k) .lt. threshold) cycle
                  ! f = (Lmax^2-3)/(Lmax^2 - tr(C))
                  f = (this%Lmax**2 - 3.0_WP) / (this%Lmax**2 - (this%SCrec2(i,j,k,1) + this%SCrec2(i,j,k,4) + this%SCrec2(i,j,k,6)))
                  ...
                  this%SCrec(i,j,k,1) = this%SCrec(i,j,k,1)*exp(coeff) + (1.0_WP/f)*(1.0_WP - exp(coeff))*phi1
                  this%SCrec(i,j,k,4) = this%SCrec(i,j,k,4)*exp(coeff) + (1.0_WP/f)*(1.0_WP - exp(coeff))*phi1
                  this%SCrec(i,j,k,6) = this%SCrec(i,j,k,6)*exp(coeff) + (1.0_WP/f)*(1.0_WP - exp(coeff))*phi1
                  this%SCrec(i,j,k,2) = this%SCrec(i,j,k,2)*exp(coeff)
                  this%SCrec(i,j,k,3) = this%SCrec(i,j,k,3)*exp(coeff)
                  this%SCrec(i,j,k,5) = this%SCrec(i,j,k,5)*exp(coeff)
               end do
   ...
   end subroutine get_relax_analytical2
